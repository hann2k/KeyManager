...
1단계는 Named Pipe 로컬 전용이다. 다음 계획:

1. **TCP 버전** — `ISecretTransport`(문서 §7)로 갈아끼우는 TCP/TLS 전송 어댑터. 다른 머신·컨테이너·WSL의 소비 앱이 접속 가능. §9 프로토콜을 재사용하되 TLS와 시간 기반 코드의 시계 오차 허용을 추가.
2. **클라우드 버전** — zero-knowledge 하이브리드(문서 §11). 클라우드는 **암호문 금고만 동기화**하고 복호화는 로컬 에이전트에 유지. sync 백엔드·충돌/버전 관리·백업/복구(복구 코드) 추가.
